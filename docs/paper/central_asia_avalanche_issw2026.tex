...
Across 23 modeled stations, 24-hour new snow and maximum air temperature ($\TAmax$) emerge as the most consistent predictors. Positive $\TAmax$ coefficients suggest that warm, likely rain-influenced storms are at least as important as new-snow loading in driving avalanche activity.
...
\begin{figure}[ht]
  ...
  \caption{A valley-floor avalanche photo of the kind local observers post to Facebook and Instagram and AKAH compiles into the observation dataset. The debris is visible, but the start zone is not — the gap this paper's start-zone matching (Section \ref{sec:obs-matching}) is built to close.}
